...
async function downloadCSV(filters: { type?: string; from?: string; to?: string }, token: string) {
  ...
  a.href = url
  a.download = `moneytrack_export.csv`
  a.click()
        ...
        <form onSubmit={e => { e.preventDefault(); mutation.mutate() }} className={styles.form}>
          ...
          <div className={styles.formGrid}>
            <label className={styles.label}>
              Importe (€)
              <input className={styles.input} type="number" min="0.01" step="0.01" value={form.amount} onChange={set('amount')} required placeholder="0,00" />
            </label>
            <label className={styles.label}>
              Fecha
          ...
          <label className={styles.label}>
            Descripción
            <input className={styles.input} type="text" value={form.description} onChange={set('description')} placeholder="Opcional" maxLength={255} />
          </label>
...
export function TransactionsPage() {
  ...
  async function handleExport() {
    ...
    setExporting(true)
    try {
      await downloadCSV({ type: filterType || undefined, from: filterFrom || undefined, to: filterTo || undefined }, session.access_token)
    } finally {
      setExporting(false)
    }
  }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.filters}>
          ...
          <input className={styles.filterDate} type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} placeholder="Desde" title="Desde" />
          <input className={styles.filterDate} type="date" value={filterTo}   onChange={e => setFilterTo(e.target.value)}   placeholder="Hasta" title="Hasta" />
      ...
      <div className={styles.tableWrap}>
        {isLoading && (
          <div className={styles.tableEmpty}><span>Cargando movimientos...</span></div>
        )}
              ...
              {filtered.map(tx => (
                <tr key={tx.id} className={styles.tableRow} onClick={() => openEdit(tx)}>
                  ...
                  <td className={styles.tdCat}>
                    {tx.category ? (
                      <span className={styles.catChip} style={{ '--cat-color': tx.category.color ?? 'var(--color-text-muted)' } as any}>
                        {tx.category.name}
                      </span>
                  ...
                  <td className={styles.tdAction}>
                    <button className={styles.iconBtn} onClick={e => { e.stopPropagation(); openEdit(tx) }}>
      ...
      {modalOpen && (
        <TransactionModal transaction={editing} categories={categories} onClose={closeModal} />
      )}
    </div>
  )
}
